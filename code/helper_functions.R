...
# Extract the estimate, SE, and formatted CI for the specified terms from the 
# umxRAM output and round to the specified number of digits 
f_ests_fit <- function(umx_out, terms, d) {
  tb <- get_path_ests_umx(umx_out = umx_out, terms = terms)
  fit_i <- fit_umx(summary(umx_out), d)
  out <- c(c(rbind(rnd(tb[ ,"Est"], d), 
                   rnd(tb[ ,"SE"], d), 
                   f_ci(tb[, c("LB", "UB")], d))),
           fit_i)
  return(out)
...
build_mnsd_table <- function(capt = NULL, dig) {
  ...
  std_v <- c("self_efficacy_ao_s", "self_efficacy_mim_s", "self_efficacy_mn_s")
  ...
  tab_kbl <-
    ...
    add_header_above(c(" " = 1, "Standardized" = 6, "Unstandardized" = 6)) %>%
...
build_probit_table <- function() {
  ...
  kbl(tab_probit, align = "c", booktabs = T)  %>%
    ...
    pack_rows(index = c("(intercept)" = 3, "self-efficacy"= 3, "ELS"= 3, 
                        "Model information" = 4)) %>%
    add_header_above(c(" ", "FS (2S-PA)" = 2, "FS (PA)" = 2, " " = 1)) %>%
    kable_styling(latex_options = "scale_down") %>% 
    kableExtra::footnote(general = "'self-efficacy' refers to standardized Bartlett factor scores (FS) computed using the approximate and partial invariance models determined with the alignment optimization (AO) and measurement invariance modeling (MIM) approaches respectively, or to standardized observed composite mean scores (CMS) on the math self-efficacy test items. FS were corrected for measurement noninvariance and unreliability following a two-stage path analysis (2S-PA) approach or only for measurement noninvariance (PA). CMS do not account for noninvariance or unreliability. ELS refers to sample membership (ELS: 1, HSLS: 0). Values in this table are reported with three decimal places to highlight the subtle differences across conditions.",
                         threeparttable = TRUE, footnote_as_chunk = TRUE)
}
...
build_prob_at_score_levels_table <- function() {
  ...
  ao_2spa_f <- function(score, ELS) pnorm(b[1, 1] + b[2, 1] * score + b[3, 1] * ELS)
  mim_2spa_f <- function(score, ELS) pnorm(b[1, 2] + b[2, 2] * score + b[3, 2] * ELS)
  ao_f <- function(score, ELS) pnorm(b[1, 3] + b[2, 3] * score + b[3, 3] * ELS)
  mim_f <- function(score, ELS) pnorm(b[1, 4] + b[2, 4] * score + b[3, 4] * ELS)
  mn_f <- function(score, ELS) pnorm(b[1, 5] + b[2, 5] * score + b[3, 5] * ELS)
  
  vec_p <- function(s, e) {
    df <- cbind(ao_2spa_f(s, e), mim_2spa_f(s, e), ao_f(s, e), mim_f(s, e), 
                mn_f(s, e))
    colnames(df) <- models
    df * 100
  }
...
scatter_p <- function(dataset, x, y, xlab, ylab, title, ref_line = TRUE) {
  ...
  p <- ggplot(dataset, aes(x, y, col = sample)) +
    xlab(xlab) +  ylab(ylab) + ggtitle(title) + xlim(-2,2)+
...
# AO vs. MIM
# scatter_p(df, y = df$self_efficacy_ao_s, x = df$self_efficacy_mim_s, 
#           xlab = "math self-efficacy FS (MIM)",
#           ylab = "math self-efficacy FS (AO)",
#           "Correlation of Standardized Bartlett Factor Scores (FS) from AO and MIM")
